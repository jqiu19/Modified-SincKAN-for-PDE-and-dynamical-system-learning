...
def get_config():
    """ModifiedSincKAN stable sinc with u/v using degree=12, len_h=2."""
    config = get_base_config()

    config.swanlab.name = "modifiedsinckan_uv_same_d12h2_stablesinc"
    ...
    config.arch.len_h = 1
    config.arch.u_degree = 12
    config.arch.u_len_h = 2
    config.arch.v_degree = 12
    config.arch.v_len_h = 2
