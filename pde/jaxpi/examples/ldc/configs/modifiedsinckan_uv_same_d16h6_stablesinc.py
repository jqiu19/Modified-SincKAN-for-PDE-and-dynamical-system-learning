...
def get_config():
    """ModifiedSincKAN stable sinc with u/v using degree=16, len_h=6."""
    config = get_base_config()

    config.swanlab.name = "modifiedsinckan_uv_same_d16h6_stablesinc"
    ...
    config.arch.u_len_h = 6
    config.arch.v_degree = 16
    config.arch.v_len_h = 6
