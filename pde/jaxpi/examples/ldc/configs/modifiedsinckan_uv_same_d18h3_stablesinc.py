...
def get_config():
    """ModifiedSincKAN stable sinc with u/v using degree=18, len_h=3."""
    config = get_base_config()

    config.swanlab.name = "modifiedsinckan_uv_same_d18h3_stablesinc"
    ...
    config.arch.len_h = 1
    config.arch.u_degree = 18
    config.arch.u_len_h = 3
    config.arch.v_degree = 18
