...
def get_config():
    """ModifiedSincKAN stable sinc with u/v using degree=15, len_h=3."""
    config = get_base_config()

    config.swanlab.name = "modifiedsinckan_uv_same_d15h3_stablesinc"
    ...
    config.arch.len_h = 1
    config.arch.u_degree = 15
    config.arch.u_len_h = 3
    config.arch.v_degree = 15
