    ...
    def __call__(self, state, batch, x_star, y_star, U_ref, nu):
        ...
        if self.config.logging.log_nonlinearities:
            ...
            sinc_layer_keys = [
                ...
                if key.endswith(
                    tuple(
                        [f"SincLayers_{i}" for i in range(2)]
                    )
                )
            ]
            for i, key in enumerate(sinc_layer_keys):
                self.log_dict[f"weight_{i}_1"] = state.params["params"][key]["weight1"]
                self.log_dict[f"weight_{i}_2"] = state.params["params"][key]["weight2"]
