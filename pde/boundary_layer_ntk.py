...
def get_weights(model, ob_x, ob_sup, frozen_para, lams, alpha=0.1):
    ...
    l_r = jnp.linalg.norm(J_rs, ord=jnp.inf)

    l_b = jnp.mean(lams[0]*jnp.abs(J_bs))
...
def train(key):
    ...
    path = f'{args.datatype}_{args.network}_{args.seed}_{args.alpha}_ntk.eqx'
    eqx.tree_serialise_leaves(path, model)
    path = f'{args.datatype}_{args.network}_{args.seed}_{args.alpha}_ntk.npz'
