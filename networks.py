...
def get_network(args, input_dim, output_dim, interval, normalizer, keys):
    if args.network == 'mlp':
        ...
        model = modifiedMLP(input_dim=input_dim, output_dim=output_dim, N_features=args.features, N_layers=args.layers,
                            normalizer=normalizer, activation=args.activation,
                            key=keys[0])
    elif args.network in ('modifiedsinckan', 'modifiedkan'):
        model = modifiedSincKAN(
            input_dim=input_dim,
            ...
            init_h=args.init_h,
            skip=bool(args.skip),
            uv_activation=getattr(args, "uv_activation", "none"),
            initialization=getattr(args, "initialization", "Xavier"),
            gate_mode=getattr(args, "gate_mode", "separate_residual"),
            aug_scale=getattr(args, "aug_scale", 1.0),
            u_degree=getattr(args, "u_degree", None),
            v_degree=getattr(args, "v_degree", None),
            g_degree=getattr(args, "g_degree", None),
            u_len_h=getattr(args, "u_len_h", None),
            v_len_h=getattr(args, "v_len_h", None),
            g_len_h=getattr(args, "g_len_h", None),
            interval=interval,
            uv_network=getattr(args, "uv_network", "sinckan"),
        )
...
class modifiedSincKAN(eqx.Module):
    matrices: list
    biases: list
    u_layer: "SincLayers"
    v_layer: "SincLayers"
    ...
    def __init__(self, input_dim, output_dim, N_features, N_layers, normalizer, key, activation='tanh', degree=16,
                 len_h=3, decay='inverse', init_h=2.0, skip=True, uv_activation='none',
                 initialization='Xavier', gate_mode='separate_residual', aug_scale=1.0,
                 u_degree=None, v_degree=None, g_degree=None,
                 u_len_h=None, v_len_h=None, g_len_h=None, interval=None, uv_network='sinckan'):
        keys = random.split(key, N_layers + 4)
        features = [input_dim, ] + [N_features, ] * (N_layers - 1) + [output_dim, ]
        self.matrices = [
            random.normal(k, (f_in, f_out)) / jnp.sqrt((f_in + f_out) / 2.0)
            for f_in, f_out, k in zip(features[:-1], features[1:], keys[:N_layers])
        ]
        self.biases = [jnp.zeros((f_out,)) for f_out in features[1:]]

        gate_dim = features[1] if len(features) > 1 else output_dim
        u_degree = degree if u_degree is None else u_degree
        v_degree = degree if v_degree is None else v_degree
        g_degree = degree if g_degree is None else g_degree
        u_len_h = len_h if u_len_h is None else u_len_h
        v_len_h = len_h if v_len_h is None else v_len_h
        g_len_h = len_h if g_len_h is None else g_len_h
        uv_interval = [-1.0, 1.0] if interval is None else interval
        self.u_layer = self._build_uv_layer(
            uv_network, input_dim, gate_dim, u_degree, u_len_h, uv_interval,
            keys[N_layers], init_h, uv_activation, decay, skip, initialization
        )
        self.v_layer = self._build_uv_layer(
            uv_network, input_dim, gate_dim, v_degree, v_len_h, uv_interval,
            keys[N_layers + 1], init_h, uv_activation, decay, skip, initialization
        )
        self.g_layer = self._build_uv_layer(
            uv_network, input_dim, gate_dim, g_degree, g_len_h, uv_interval,
            keys[N_layers + 2], init_h, uv_activation, decay, skip, initialization
        )
        self.gate_matrix = random.normal(keys[N_layers + 3], (gate_dim, gate_dim)) / jnp.sqrt(gate_dim)
        self.gate_bias = jnp.zeros((gate_dim,))
        ...
        self.uv_network = uv_network

    def _build_uv_layer(self, uv_network, input_dim, output_dim, degree, len_h, interval, key, init_h, activation,
                        decay, skip, initialization):
        if uv_network == 'linear':
            return LinearLayer(input_dim, output_dim, key)
        if uv_network == 'mlp':
            return LinearLayer(input_dim, output_dim, key)
        if uv_network == 'kan':
            return KANLayers(input_dim, output_dim, degree, interval, key)
        if uv_network == 'chebykan':
            return ChebyLayers(input_dim, output_dim, degree, key)
        if uv_network == 'sinckan':
            return SincLayers(
                input_dim=input_dim, output_dim=output_dim, degree=degree,
                key=key, init_h=init_h, len_h=len_h, activation=activation,
                decay=decay, skip=skip, initialization=initialization
            )
        raise ValueError(f'Unsupported uv_network: {uv_network}')

    def __call__(self, input, frozen_para):
        input = self.normalizer[0](input)

        u = self.activation(self.u_layer(input, frozen_para['u']))
        v = self.activation(self.v_layer(input, frozen_para['v']))
        g_input = self.g_layer(input, frozen_para['g'])

        f = input @ self.matrices[0] + self.biases[0]
        for i in range(1, len(self.matrices)):
            h = self.activation(f)
            if self.gate_mode in ('original', 'original_modifiedmlp_gate'):
                mixed = h * u + (1.0 - h) * v
            elif self.gate_mode == 'sigmoid':
                gate = jax.nn.sigmoid(u)
                mixed = gate * h + (1.0 - gate) * v
            elif self.gate_mode == 'residual':
                mixed = h + h * u + (1.0 - h) * v
            elif self.gate_mode == 'separate_residual':
                gate = jax.nn.sigmoid(g_input + h @ self.gate_matrix + self.gate_bias)
                candidate = gate * u + (1.0 - gate) * v
                mixed = h + self.aug_scale * candidate
            elif self.gate_mode == 'separate_sigmoid':
                gate = jax.nn.sigmoid(g_input + h @ self.gate_matrix + self.gate_bias)
                candidate = gate * u + (1.0 - gate) * v
                mixed = (1.0 - self.aug_scale) * h + self.aug_scale * candidate
            else:
                mixed = h * u + (1.0 - h) * v
            f = mixed @ self.matrices[i] + self.biases[i]
        return f

    def get_frozen_para(self):
        return {
            'u': self.u_layer.get_frozen_para(),
            'v': self.v_layer.get_frozen_para(),
            'g': self.g_layer.get_frozen_para(),
        }
...
class LinearLayer(eqx.Module):
    ...
    def __init__(self, input_dim, output_dim, key):
        self.matrix = random.normal(key, (input_dim, output_dim)) / jnp.sqrt((input_dim + output_dim) / 2.0)
