...
parser.add_argument("--datatype", type=str, default='multimodal1', help="type of data")
...
parser.add_argument("--ntrain", type=int, default=2000, help="the number of training dataset for each epochs")
...
parser.add_argument("--wandb_group", type=str, default='approximation_hd_implicit', help="wandb group name")
...
def train(key):
    ...
    wandb_run = init_wandb_run(args, "approximation_hd_implicit",
                               extra_config={"input_dim": input_dim, "output_dim": output_dim})
    ...
    lr_schedule = optax.join_schedules(
        schedules=[
            optax.exponential_decay(
                ...
                transition_steps=N_drop,
                decay_rate=0.95
            ),
            optax.exponential_decay(
                init_value=learning_rate * (0.95 ** 6),  # Match the last LR of first phase
                transition_steps=N_drop,
                decay_rate=0.7
            )
        ],
        boundaries=[60000]  # Boundary at 100,000 iterations
    )
    ...
    for j in range(ite * N_epochs):
        ...
        if j % N_epochs == 0: # for every N_epochs epochs, resample the training points and labels, evaluate the trained model on x_test, record and print the relative error and mse error
            ...
            print(f'ite:{j},testing rmse:{mse_error**0.5:.2e},relative:{relative_error:.2e}')
            log_wandb(wandb_run, {
    ...
    relative_errors.append(relative_error)
    mse_errors.append(mse_error)
    print(f'ite:{ite * N_epochs},testing rmse:{rmse_error:.2e},relative:{relative_error:.2e}')
