...
from utils import normalization, normalization_by_points
...
parser.add_argument("--uv_network", type=str, default='sinckan', help="u/v/g subnet of modifiedsinckan: linear, mlp, kan, chebykan, sinckan")
...
parser.add_argument("--wandb_group", type=str, default='interpolation_hd', help="wandb group name")
parser.add_argument("--result_tag", type=str, default='', help="optional suffix for result CSV names")
args = parser.parse_args()

os.environ['CUDA_VISIBLE_DEVICES'] = str(args.device)


def build_artifact_stem(args):
    stem = f"{args.datatype}_{args.network}"
    if args.network in ("modifiedsinckan", "modifiedkan"):
        stem += f"_{args.uv_network}_{args.gate_mode}"
    stem += f"_{args.seed}_{args.dim}"
    return stem
...
def train(key):
    ...
    wandb_run = init_wandb_run(args, "interpolation_hd", extra_config={"input_dim": input_dim, "output_dim": output_dim})
    ...
    relative_errors.append(relative_error)
    mse_errors.append(mse_error)
    print(f'ite:{ite * N_epochs},testing mse:{mse_error:.2e},rmse:{rmse_error:.2e},relative:{relative_error:.2e}')



    # save model and results
    stem = build_artifact_stem(args)
    path = f'{stem}.eqx'
    ...
    append_result_row(save_here, header, [
        args.datatype, args.network, args.seed, history[-1], np.sum(np.array(T)), ite * N_epochs,
        mse_errors[-1], jnp.sqrt(mse_errors[-1]), relative_errors[-1]
    ])
